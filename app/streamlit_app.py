...
def build_forecast_features(future_weather: pd.DataFrame, history: pd.DataFrame, feature_names: list):
    ...
    last_aqi = float(history["aqi"].iloc[-1]) if "aqi" in history.columns and len(history) else 100.0
    ...
    df_model  = df[available].fillna(0.0)
...
with st.sidebar:
    ...
    st.caption("Data sourced from AQICN API (Chinese AQI standard).")
    st.caption("Data refreshes every hour automatically.")

# ── MAIN ──────────────────────────────────────────────────────────────────────
st.title(f"🌫️ AQI Forecast — {city_input}")
st.caption(f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
st.info("ℹ️ AQI data sourced from AQICN which uses the Chinese AQI standard. Values may differ from Google/Breezometer which uses the US EPA standard.")
    ...
    try:
        ...
        try:
            if len(history_df) > 0:
                ...
                st.info("No historical data available for EDA yet.")
