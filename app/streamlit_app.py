...
st.set_page_config(page_title="AQI Forecast", page_icon="🌫️", layout="wide", initial_sidebar_state="expanded")
...
def load_model_and_features():
    ...
    model_meta = mr.get_model("aqi_predictor", version=6)
...
def fetch_future_weather() -> pd.DataFrame:
    url = ("https://api.open-meteo.com/v1/forecast?latitude=33.72148&longitude=73.04329"
           "&hourly=temperature_2m,relativehumidity_2m,windspeed_10m,precipitation&forecast_days=4&timezone=UTC")
...
def fetch_real_aqi(token: str) -> float:
    try:
        url  = f"https://api.openweathermap.org/data/2.5/air_pollution?lat=33.72148&lon=73.04329&appid={token}"
...
def build_forecast_features(future_weather, history, feature_names):
    ...
    df["aqi_lag_1h"] = last_aqi; df["aqi_lag_3h"] = last_aqi
    df["aqi_lag_6h"] = last_aqi; df["aqi_lag_24h"] = last_aqi
    df["aqi_change_rate"] = 0.0
    df["aqi_rolling_6h"]  = last_aqi; df["aqi_rolling_24h"] = last_aqi
    available = [c for c in feature_names if c in df.columns]
    return df, df[available].fillna(0.0), available
...
with st.sidebar:
    ...
    st.caption("ℹ️ AQI data from OpenWeather (US EPA standard).")
...
with st.spinner("Loading model and data..."):
    try:
        ...
        with col1:
            st.markdown(f"""<div style='background:{color};border-radius:12px;padding:20px;text-align:center;'>
                <h1 style='color:black;margin:0;font-size:64px'>{int(current_aqi)}</h1>
                <h3 style='color:black;margin:4px'>{label}</h3>
                <p style='color:black;font-size:13px'>{desc}</p></div>""", unsafe_allow_html=True)
        with col2:
        ...
        fig = go.Figure()
        for bname, bcol, lo, hi in [("Good","#00e400",0,50),("Moderate","#ffff00",50,100),
        ...
        fig.update_layout(xaxis_title="Time", yaxis_title="AQI", yaxis_range=[0, 400],
                          legend=dict(orientation="h", y=1.08), height=400, margin=dict(l=0,r=0,t=20,b=0))
        ...
        st.dataframe(daily.style.format({"Avg AQI":"{:.0f}","Min AQI":"{:.0f}","Max AQI":"{:.0f}"}), use_container_width=True)

        with st.expander("🌤️ Weather Forecast Context"):
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(x=df_future["timestamp"], y=df_future["temperature"], name="Temp (°C)", yaxis="y1"))
            fig2.add_trace(go.Bar(x=df_future["timestamp"], y=df_future["precipitation"], name="Precipitation (mm)", yaxis="y2", opacity=0.4))
            fig2.update_layout(yaxis=dict(title="Temperature °C"), yaxis2=dict(title="Precipitation mm", overlaying="y", side="right"),
        ...
        with st.expander("🔍 Feature Importance (SHAP)"):
            try:
                ...
                shap_df = pd.DataFrame({"Feature": used_features, "Importance": importance}).sort_values("Importance", ascending=True)
                fig_shap = go.Figure()
                fig_shap.add_trace(go.Bar(x=shap_df["Importance"], y=shap_df["Feature"], orientation="h", marker_color="#EF553B"))
                fig_shap.update_layout(title="SHAP Feature Importance", xaxis_title="Mean |SHAP Value|",
                                       yaxis_title="Feature", height=500, margin=dict(l=0,r=0,t=40,b=0))
